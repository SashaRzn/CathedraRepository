...
{
	public partial class ControlLoad : Form
	{
		...
		private void InitDataGrid()
		{
            ...
            {
                var query = from s in database.CourseInWork
                            where s.SchoolYearID == schoolYear && s.Semestr == semestrValue && s.FormStudy == formStudyValue
                            select s;
		...
		private void CourseEdit()
		{
            if (ctlCourseInWork.SelectedRows[0].Cells["idColumn"].Value != null)
            {
                ...
                CourseInWork courseInWork = (from ciw in database.CourseInWork
                                             where ciw.ID == idCoourseInWork
                                             select ciw).SingleOrDefault();
                CourseInWorkForm formCourseInWork = new CourseInWorkForm(courseInWork, bsSemestr, formStudyValue, database);
		...
		{
			CourseInWorkForm formCourseInWork = new CourseInWorkForm(bsSemestr, formStudyValue, database);
			if (formCourseInWork.ShowDialog(this) == DialogResult.OK)
			{
				int courseId = formCourseInWork.СourseId;
				int employeeId = formCourseInWork.EmployeeId;
